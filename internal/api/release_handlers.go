...
func (s *Server) ContaminationHandler(w http.ResponseWriter, r *http.Request) {
	...
	result, err := s.workflow.WithContext(r.Context()).RecordContamination(cmd)
	if err != nil {
		writeError(w, r, err)
		return
	}
...
func (s *Server) ReviewHandler(w http.ResponseWriter, r *http.Request) {
	...
	result, err := s.workflow.WithContext(r.Context()).Review(cmd)
	if err != nil {
		writeError(w, r, err)
		return
	}
...
func (s *Server) ReleaseHandler(w http.ResponseWriter, r *http.Request) {
	...
	result, err := s.workflow.WithContext(r.Context()).Release(cmd)
	if err != nil {
		writeError(w, r, err)
		return
	}
